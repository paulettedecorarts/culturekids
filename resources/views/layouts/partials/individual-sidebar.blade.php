...
>
    <div class="th-sidebar-head">
        <a href="{{ route('individual.dashboard') }}" class="th-sidebar-brand" title="{{ config('brand.name') }}">
            <span class="th-sidebar-mark" aria-hidden="true">
                <x-brand-logo variant="mark" />
            </span>
            <div class="th-sidebar-brand-text">
                {{ __('Learner Hub') }}
                <span>{{ auth()->user()->name ?? __('Learner') }}</span>
            </div>
        </a>
        <button
            type="button"
    ...
    <nav class="th-sidebar-nav" aria-label="{{ __('Individual navigation') }}">
        {!! $thNavLink(route('individual.dashboard'), '🏠', __('Dashboard'), request()->routeIs('individual.dashboard')) !!}
        ...
        <div class="th-nav-section"><span class="th-nav-section-text">{{ __('Account') }}</span></div>
        {!! $thNavLink(route('profile'), '⚙️', __('Profile settings'), request()->routeIs('profile')) !!}
    </nav>
