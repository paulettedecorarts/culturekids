<div class="clan-form-page">
    <style>
    .clan-form-page .cf-card { background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.08);border-radius:12px;padding:24px;margin-bottom:20px; }
    .clan-form-page .cf-title { font-size:11px;font-weight:700;color:rgba(255,255,255,.5);text-transform:uppercase;letter-spacing:.6px;margin-bottom:18px; }
    .clan-form-page .cf-label { display:block;font-size:11px;font-weight:600;color:rgba(255,255,255,.6);text-transform:uppercase;letter-spacing:.4px;margin-bottom:6px; }
    .clan-form-page .cf-input { display:block;width:100%;box-sizing:border-box;padding:9px 12px;border-radius:8px;border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.05);color:#fff;font-size:13px;font-family:var(--font-admin,inherit);transition:border-color .2s; }
    .clan-form-page .cf-input:focus { outline:none;border-color:rgba(212,160,23,.6);background:rgba(255,255,255,.07); }
    .clan-form-page .cf-input::placeholder { color:rgba(255,255,255,.3); }
    .clan-form-page select.cf-input { background:#1a2744;color:#fff;color-scheme:dark; }
    .clan-form-page select.cf-input option { background:#1a2744;color:#fff; }
    ...
    <form wire:submit="save">
        ...
        <div class="cf-card">
                    ...
                    <div style="position:relative">
                        <div style="display:flex;gap:6px;align-items:center">
                            <button type="button" wire:click="openEmojiPicker"
                                style="flex:1;height:36px;border-radius:8px;border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.05);color:#fff;font-size:22px;cursor:pointer;{{ $showEmojiPicker ? 'border-color:rgba(212,160,23,.6)' : '' }}">
                                {{ $totem_emoji ?: '＋' }}
                            </button>
                            @if($totem_emoji)
                                <button type="button" wire:click="$set('totem_emoji', '')"
                                    style="width:36px;height:36px;border-radius:8px;border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.05);color:rgba(255,255,255,.5);cursor:pointer;font-size:12px">✕</button>
                            @endif
                        </div>

                        @if($showEmojiPicker)
                        <div style="position:absolute;z-index:200;top:calc(100% + 8px);left:0;background:#1a2744;border:1px solid rgba(255,255,255,.15);border-radius:12px;padding:14px;width:300px;box-shadow:0 12px 40px rgba(0,0,0,.6)">
                            {{-- Category tabs --}}
                            ...
                            <div style="margin-top:8px;text-align:right">
                                <button type="button" wire:click="$set('showEmojiPicker', false)" style="font-size:11px;color:rgba(255,255,255,.5);background:none;border:none;cursor:pointer">Close</button>
                            </div>
                        </div>
                        @endif
                    </div>
            ...
            <div class="cf-grid-2" style="margin-bottom:16px">
                <div class="cf-field">
                    <label class="cf-label">Description <span style="color:rgba(255,255,255,.35);font-weight:400;text-transform:none;font-size:10px">brief</span></label>
                    <textarea wire:model="description" class="cf-input" rows="3" placeholder="Brief description of the clan..."></textarea>
                </div>
                <div class="cf-field">
                    <label class="cf-label">Cover Image <span style="color:rgba(255,255,255,.35);font-weight:400;text-transform:none;font-size:10px">max 5MB</span></label>
                    <input wire:model="cover_image_file" type="file" class="cf-input" accept="image/*">
                    @if($clan && $clan->cover_image_path)
                        <img src="{{ asset('storage/' . $clan->cover_image_path) }}" style="margin-top:6px;max-width:80px;border-radius:4px;border:1px solid rgba(255,255,255,.1)">
                    @endif
                </div>
            </div>

            <div class="cf-field" style="margin-bottom:16px">
                <label class="cf-label">History <span style="color:rgba(255,255,255,.35);font-weight:400;text-transform:none;font-size:10px">detailed narrative</span></label>
            ...
            <label style="display:flex;align-items:center;gap:8px;cursor:pointer">
                ...
                <span style="font-size:12px;color:rgba(255,255,255,.7)">Active (visible in the app)</span>
            </label>
        </div>
