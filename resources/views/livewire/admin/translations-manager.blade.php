<div>
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:var(--sp-5);flex-wrap:wrap;gap:var(--sp-3)">
        ...
        <div style="display:flex;gap:var(--sp-2);align-items:center">
            <button wire:click="createTag" class="btn btn-primary btn-sm" style="background:var(--clay-red); border:none; color:#fff; padding: var(--sp-2) var(--sp-4); border-radius: var(--r-full); font-weight:700; cursor:pointer;">+ Add Translation</button>
        </div>
    </div>
    ...
    <div style="display:flex; gap:var(--sp-2); margin-bottom:var(--sp-4);">
        <input wire:model.live.debounce.300ms="search" type="text" placeholder="Search word, translation, comic, tribe..." style="background:rgba(255,255,255,.05); border:1px solid rgba(255,255,255,.1); border-radius:var(--r-full); padding:var(--sp-2) var(--sp-4); color:#fff; font-size:12px; outline:none; min-width:300px;">
        <select wire:model.live="statusFilter" style="background:rgba(255,255,255,.05); border:1px solid rgba(255,255,255,.1); border-radius:var(--r-full); padding:var(--sp-2) var(--sp-4); color:#fff; font-size:12px; outline:none;">
    ...
    <div class="sa-table-wrap">
        ...
        @forelse($tags as $tag)
            <div class="sa-table-row" style="grid-template-columns:1.4fr 1.6fr 1.6fr 1.2fr 120px">
                <span style="font-size:12px;color:#fff;font-weight:700">{{ $tag->word }}</span>
                ...
                <span style="font-size:12px;color:rgba(255,255,255,.7)">{{ $tag->phonetic ?: '—' }}</span>
        ...
        @empty
            <div style="padding:22px;color:rgba(255,255,255,.5)">No translation tags found.</div>
        @endforelse
    </div>
    ...
    @if($showModal)
        <div style="position:fixed;inset:0;background:rgba(0,0,0,0.8);z-index:1000;display:flex;align-items:center;justify-content:center;padding:24px">
            <div style="background:#111827;border:1px solid rgba(255,255,255,.12);border-radius:20px;width:min(760px,100%);padding:20px">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">
                    <div style="font-size:16px;font-weight:800;color:#fff">{{ $editing ? 'Edit Translation' : 'Add Translation' }}</div>
                    <button wire:click="closeModal" class="btn btn-ghost btn-sm">Close</button>
                </div>

                <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
                    <div style="grid-column:1/-1">
                        <label style="display:block;font-size:11px;color:rgba(255,255,255,.55);margin-bottom:4px">Story</label>
                        <select wire:model.live="comic_id" style="width:100%;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.12);border-radius:10px;padding:10px;color:#fff;color-scheme:dark">
                            <option value="">Select story</option>
                            @foreach($storyOptions as $story)
                                <option value="{{ $story->id }}" style="background:#111827;color:#fff">
                    ...
                    <div style="grid-column:1/-1">
                        <label style="display:block;font-size:11px;color:rgba(255,255,255,.55);margin-bottom:4px">Panel</label>
                        <select wire:model="panel_id" style="width:100%;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.12);border-radius:10px;padding:10px;color:#fff;color-scheme:dark">
                            <option value="">Select panel</option>
                            @foreach($panelOptions as $panel)
                                <option value="{{ $panel->id }}" style="background:#111827;color:#fff">
                    ...
                    <div>
                        <label style="display:block;font-size:11px;color:rgba(255,255,255,.55);margin-bottom:4px">Word</label>
                        <input wire:model="word" type="text" style="width:100%;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.12);border-radius:10px;padding:10px;color:#fff">
                        @error('word') <div style="font-size:10px;color:#fda4af;margin-top:4px">{{ $message }}</div> @enderror
                    </div>

                    <div>
                        <label style="display:block;font-size:11px;color:rgba(255,255,255,.55);margin-bottom:4px">Translation</label>
                        <input wire:model="translation" type="text" style="width:100%;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.12);border-radius:10px;padding:10px;color:#fff">
                        @error('translation') <div style="font-size:10px;color:#fda4af;margin-top:4px">{{ $message }}</div> @enderror
                    </div>

                    <div style="grid-column:1/-1">
                        <label style="display:block;font-size:11px;color:rgba(255,255,255,.55);margin-bottom:4px">Phonetic</label>
                        <input wire:model="phonetic" type="text" style="width:100%;background:rgba(255,255,255,.06);border:1px solid rgba(255,255,255,.12);border-radius:10px;padding:10px;color:#fff">
