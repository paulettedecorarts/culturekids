        ...
        @forelse($modules as $module)
            <div style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.07);border-radius:var(--r-xl);padding:var(--sp-5)">
                <div style="display:flex;align-items:center;gap:var(--sp-4)">
                    <div style="width:48px; height:48px; border-radius:14px; background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.1); display:flex; align-items:center; justify-content:center; font-size:24px">
                        {{ $module->icon ?? '🧩' }}
                    </div>
                    <div style="flex:1">
                        <div style="font-family:var(--font-display);font-size:18px;font-weight:700;color:#fff">{{ $module->name }}</div>
                        <div style="font-size:12px;color:rgba(255,255,255,.4)">{{ $module->key }} · Sort: {{ $module->sort_order }}</div>
                    </div>
                ...
                @if($module->description)
                    <div style="margin-top:var(--sp-3);padding-top:var(--sp-3);border-top:1px solid rgba(255,255,255,.05);font-size:13px;color:rgba(255,255,255,.5)">
                        {{ $module->description }}
                    </div>
                @endif
            </div>
        ...
        <div style="position:fixed; inset:0; background:rgba(0,0,0,0.8); backdrop-filter:blur(10px); z-index:1000; display:flex; align-items:center; justify-content:center; padding:40px">
            <div style="background:var(--indigo-night); width:100%; max-width:600px; border:1px solid rgba(255,255,255,0.15); border-radius:32px; box-shadow:0 40px 100px rgba(0,0,0,0.5); overflow:hidden" onclick="event.stopPropagation()">
                <div style="padding:32px; border-bottom:1px solid rgba(255,255,255,0.1); display:flex; align-items:center; justify-content:space-between">
                    <div>
                        <h2 style="font-family:var(--font-display); font-size:24px; color:#fff">{{ $editing ? 'Edit Module' : 'New Module' }}</h2>
                        <div style="font-size:11px; color:rgba(255,255,255,0.4); text-transform:uppercase; font-weight:800; letter-spacing:1px">{{ $editing ? 'Update module configuration' : 'Register new platform feature' }}</div>
                    </div>
                ...
                <form wire:submit.prevent="save" style="padding:32px; display:grid; gap:20px">
                    <div>
                        ...
                        <input wire:model.live="name" type="text" placeholder="Tribe Directory" style="width:100%; background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.1); border-radius:12px; padding:14px; color:#fff; font-family:var(--font-admin)">
                    ...
                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:20px">
                        <div>
                            ...
                            <input wire:model="key" type="text" readonly style="width:100%; background:rgba(255,255,255,0.02); border:1px solid rgba(255,255,255,0.05); border-radius:12px; padding:14px; color:rgba(255,255,255,0.5); font-family:var(--font-admin); cursor:not-allowed">
                        ...
                        <div>
                            ...
                            <div style="position:relative">
                                <div 
                                    ...
                                    style="width:100%; background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.1); border-radius:12px; padding:14px; color:#fff; font-size:28px; text-align:center; cursor:pointer; display:flex; align-items:center; justify-content:center; min-height:56px"
                                >
                                    {{ $icon ?: '🧩' }}
                                </div>
                                        ...
                                        <div style="display:grid; grid-template-columns:repeat(8, 1fr); gap:8px">
                                            @foreach($availableIcons as $availableIcon)
                                                <button 
                                                    ...
                                                    style="background:rgba(255,255,255,0.05); border:1px solid {{ $icon === $availableIcon ? 'var(--savanna-gold)' : 'rgba(255,255,255,0.1)' }}; border-radius:8px; padding:8px; font-size:24px; cursor:pointer; transition:all 0.2s"
                    ...
                    <div>
                        ...
                        <textarea wire:model="description" rows="3" placeholder="Brief description of this module..." style="width:100%; background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.1); border-radius:12px; padding:14px; color:#fff; font-family:var(--font-admin); resize:vertical"></textarea>
                    ...
                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:20px">
                        <div>
                            ...
                            <input wire:model="sort_order" type="number" min="0" style="width:100%; background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.1); border-radius:12px; padding:14px; color:#fff; font-family:var(--font-admin)">
                        ...
                        <div>
                            ...
                            <div style="display:flex; align-items:center; gap:12px; height:48px">
                                ...
                                <label for="is_enabled" style="color:#fff; font-size:13px; cursor:pointer">Enable this module</label>
                            </div>
                        </div>
                    </div>
