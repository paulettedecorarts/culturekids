<div>
    ...
    @if($stats['usage_alert'] !== 'ok')
        <div style="background:{{ $stats['usage_alert'] === 'critical' ? 'rgba(196,75,43,.14)' : 'rgba(232,135,42,.14)' }};border:1px solid {{ $stats['usage_alert'] === 'critical' ? 'rgba(196,75,43,.45)' : 'rgba(232,135,42,.45)' }};color:var(--cms-text);padding:10px 14px;border-radius:10px;margin-bottom:var(--sp-4);font-size:12px;font-weight:700">
            Storage usage is {{ $stats['usage_percent'] }}% of quota ({{ $stats['total_storage_human'] }} / {{ $stats['quota_human'] }}).
        </div>
    @endif

    <div style="margin-bottom:var(--sp-4)">
        <div style="font-size:11px;font-weight:700;letter-spacing:1.2px;text-transform:uppercase;color: var(--cms-text-muted);margin-bottom:10px">
            Folder Usage
        </div>
        <div class="sa-stats-row" style="grid-template-columns:repeat(4,minmax(0,1fr));gap:var(--sp-3);margin-bottom:0">
            @forelse($folderBreakdown as $folder => $info)
                <div class="sa-stat" style="padding:16px 18px">
                    <div style="font-size:11px;color: var(--cms-text-muted);font-weight:700;margin-bottom:6px">{{ $folder }}</div>
                    <div style="font-size:24px;font-weight:800;color:var(--cms-text);line-height:1.1">{{ $info['count'] }}</div>
                    <div style="font-size:10px;color: var(--cms-text-muted);font-weight:700;text-transform:uppercase;letter-spacing:.7px;margin-top:4px">Files</div>
                    <div style="font-size:12px;color:var(--savanna-gold);font-weight:700;margin-top:6px">{{ $info['size_human'] }}</div>
                </div>
            @empty
                <div class="sa-stat" style="grid-column:1/-1;padding:16px 18px;color:var(--cms-text-muted)">No folder usage to display.</div>
            @endforelse
        </div>
    </div>
    ...
    <div style="display:flex; gap:var(--sp-2); margin-bottom:var(--sp-4); align-items:center; flex-wrap:wrap;">
        ...
        @if($isSuperAdmin)
            <label style="display:flex;align-items:center;gap:8px;color: var(--cms-text);font-size:12px;padding:0 6px">
        ...
        @forelse($assets as $asset)
            <div class="sa-table-row" style="grid-template-columns:80px 2fr 1fr 1fr 1fr minmax(160px, auto)">
                ...
                <div>
                    ...
                    <div style="font-size:11px;color:var(--cms-text-muted)">{{ $asset['uploaded_label'] }}</div>
                </div>
                <span style="font-size:12px;color:var(--cms-text-muted)">{{ $asset['type_label'] }}</span>
                <span style="font-size:12px;color:var(--cms-text-muted)">{{ $asset['size_human'] }}</span>
            ...
            @if(!empty($asset['linked_refs']))
                <div class="sa-table-row" style="grid-template-columns:1fr;padding-top:0">
                    <span style="font-size:11px;color: var(--cms-text-muted)">Linked refs: {{ implode(', ', $asset['linked_refs']) }}</span>
                </div>
            @endif
        @empty
            <div style="padding:22px;color:var(--cms-text-muted)">No assets found in the scanned storage paths.</div>
        @endforelse
    </div>
