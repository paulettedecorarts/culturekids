<div>
    ...
    <div style="background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.06);border-radius:var(--r-xl);overflow:hidden">
        <table style="width:100%;border-collapse:collapse;font-size:11px">
            <thead>
                <tr style="background:rgba(255,255,255,.06)">
                    <th style="padding:var(--sp-3) var(--sp-4);text-align:left;color:rgba(255,255,255,.35);font-size:9px;letter-spacing:1px;text-transform:uppercase;font-weight:700">Permission</th>
            ...
            <tbody>
                ...
                <tr style="border-bottom:1px solid rgba(255,255,255,.05)">
                    <td style="padding:var(--sp-2) var(--sp-4);color:rgba(255,255,255,.6)">{{ Str::ucfirst(str_replace('_', ' ', $permission->name)) }}</td>
                    @foreach($roles as $role)
                        ...
                        @elseif($role->hasPermissionTo($permission->name))
                            <td style="text-align:center;color:#6FA882">✓</td>
                        @else
                            <td style="text-align:center;color:rgba(255,255,255,.2)">—</td>
                        @endif
                    @endforeach
                </tr>
                @endforeach
                ...
                <tr>
                    <td colspan="{{ count($roles) + 1 }}" style="padding:var(--sp-4);text-align:center;color:rgba(255,255,255,.4)">
