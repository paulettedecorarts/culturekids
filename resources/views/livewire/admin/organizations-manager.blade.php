        ...
        @forelse($organizations as $org)
            <div style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.07);border-radius:var(--r-xl);padding:var(--sp-5)">
                <div style="display:flex;align-items:center;gap:var(--sp-4);margin-bottom:var(--sp-4)">
                    <div style="width:48px; height:48px; border-radius:14px; background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.1); display:flex; align-items:center; justify-content:center; overflow:hidden">
                        @if($org->logo_url)
                ...
                <div style="display:flex;gap:var(--sp-6);font-size:12px;color:rgba(255,255,255,.5)">
                    ...
                    <span>🌍 {{ $org->address ?: 'No location' }}</span>
                    <span>🎨 Default theme</span>
                    <span style="margin-left:auto; cursor:pointer; color:var(--clay-red); font-weight:800" onclick="confirm('Delete this entity?') || event.stopImmediatePropagation()" wire:click="delete({{ $org->id }})">Delete Org</span>
                </div>
            </div>
        @empty
        ...
        <div style="position:fixed; inset:0; background:rgba(0,0,0,0.8); backdrop-filter:blur(10px); z-index:1000; display:flex; align-items:center; justify-content:center; padding:40px">
            <div style="background:var(--indigo-night); width:100%; max-width:600px; border:1px solid rgba(255,255,255,0.15); border-radius:32px; box-shadow:0 40px 100px rgba(0,0,0,0.5); overflow:hidden" onclick="event.stopPropagation()">
                <div style="padding:32px; border-bottom:1px solid rgba(255,255,255,0.1); display:flex; align-items:center; justify-content:space-between">
                    <div>
                        <h2 style="font-family:var(--font-display); font-size:24px; color:#fff">New Organization</h2>
                        <div style="font-size:11px; color:rgba(255,255,255,0.4); text-transform:uppercase; font-weight:800; letter-spacing:1px">Register school on the platform</div>
                    </div>
                ...
                <form wire:submit.prevent="save" style="padding:32px; display:grid; grid-template-columns:1fr 1fr; gap:20px">
                    ...
                    <div>
                        <label style="display:block; font-size:11px; font-weight:800; color:var(--stone); text-transform:uppercase; margin-bottom:8px">System Code (Slug)</label>
                    ...
                    <div style="grid-column: span 2; margin-top:12px">
                        <button type="submit" class="btn btn-primary" style="width:100%; padding:16px; border-radius:14px">Complete Onboarding</button>
                    </div>
                </form>
            </div>
        </div>
