<div class="sa-tribe-detail-view">
    <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:var(--sp-8)">
        ...
        <div style="display:flex; gap:12px">
            <a href="{{ route($routePrefix . '.tribes.edit', $tribe->id) }}" class="btn" style="background: var(--cms-surface-raised); color:var(--cms-text); padding:12px 28px; border-radius:14px; text-decoration:none; border: 1px solid var(--cms-border); font-weight:800; font-size:13px">Profile Editor</a>
    ...
    <div style="display:grid; grid-template-columns:360px 1fr; gap:32px; align-items:start">
        ...
        <div style="display:flex; flex-direction:column; gap:32px">
            <div style="background:{{ $tribe->color }}; border-radius:40px; padding:48px; text-align:center; position:relative; overflow:hidden; box-shadow:0 32px 80px {{ $tribe->color.'30' }}">
                ...
                <div style="width:120px; height:120px; background: var(--cms-surface-raised); border:2px solid rgba(255,255,255,0.3); backdrop-filter:blur(10px); border-radius:40px; margin:0 auto 32px; display:flex; align-items:center; justify-content:center; font-size:64px">
                    {{ $tribe->hero_emoji ?: '🗺️' }}
                </div>
            ...
            <div style="background:var(--cms-surface-raised); border:1px solid var(--cms-border); border-radius:32px; padding:32px">
                ...
                <div style="display:flex; align-items:center; gap:16px; background:var(--cms-surface); padding:20px; border-radius:20px; border:1px solid rgba(255,255,255,0.04)">
                    ...
                    <div>
                        ...
                        <div style="font-size:11px; color:var(--cms-text-muted); font-weight:800; text-transform:uppercase">Tribal Guide</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div style="display:flex; flex-direction:column; gap:32px">
            ...
            <div style="background:var(--cms-surface-raised); border:1px solid var(--cms-border); border-radius:40px; padding:48px">
                <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:40px">
                    <div>
                        <h2 style="font-family:var(--font-display); font-size:28px; color:var(--cms-text); margin-bottom:4px">Heritage Activity Inventory</h2>
                        <div style="font-size:12px; color:var(--cms-text-muted); font-weight:700">Cultural assets assigned to the {{ $tribe->name }} archive</div>
                    </div>
                    <span style="background:var(--cms-surface-raised); border:1px solid var(--cms-border); padding:8px 16px; border-radius:12px; font-size:12px; font-weight:800; color:var(--cms-text-muted)">{{ $tribe->heritageActivities->count() }} Heritage Records · {{ $tribe->songs->count() }} Songs</span>
                </div>
                ...
                <div style="display:flex; flex-direction:column; gap:12px">
                    @forelse($tribe->heritageActivities as $activity)
                        <div style="background:var(--cms-surface); border:1px solid rgba(255,255,255,0.04); border-radius:24px; padding:24px; display:flex; align-items:center; justify-content:space-between; transition:all 0.2s" onmouseover="this.style.background='rgba(255,255,255,0.04)'" onmouseout="this.style.background='rgba(255,255,255,0.02)'">
                            <div style="display:flex; align-items:center; gap:20px">
                                ...
                                <div>
                                    <div style="font-weight:800; color:var(--cms-text); font-size:16px; margin-bottom:4px">{{ $activity->title }}</div>
                                    <div style="display:flex; gap:12px; font-size:11px; font-weight:700; color:var(--cms-text-muted)">
                    ...
                    @empty
                        <div style="padding:100px; text-align:center; opacity:0.3; background: var(--cms-surface-raised); border:1px dashed rgba(255,255,255,0.1); border-radius:32px">
