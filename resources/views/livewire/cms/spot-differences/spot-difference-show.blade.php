    ...
    <div class="sa-content-card" style="margin-bottom:var(--sp-4)">
        ...
        <div style="margin-bottom:var(--sp-4)">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--sp-3)">
                <div>
                    <div style="color:var(--cms-text-muted);font-size:12px;font-weight:600;margin-bottom:8px">Image A — Original</div>
                    @if($activity->image_a_path)
                    <div style="position:relative;display:inline-block;width:100%">
                        <img src="{{ asset('storage/' . $activity->image_a_path) }}" style="width:100%;border-radius:8px;border:1px solid var(--cms-border);display:block">
                    ...
                    @else
                        <div style="height:200px;background:var(--cms-surface-raised);border-radius:8px;border:1px dashed var(--cms-border);display:flex;align-items:center;justify-content:center;color:var(--cms-text-muted);font-size:12px">No image uploaded</div>
                    @endif
                </div>
                <div>
                    <div style="color:var(--cms-text-muted);font-size:12px;font-weight:600;margin-bottom:8px">Image B — With Differences</div>
                    @if($activity->image_b_path)
                        <img src="{{ asset('storage/' . $activity->image_b_path) }}" style="width:100%;border-radius:8px;border:1px solid var(--cms-border);display:block">
                    @else
                        <div style="height:200px;background:var(--cms-surface-raised);border-radius:8px;border:1px dashed var(--cms-border);display:flex;align-items:center;justify-content:center;color:var(--cms-text-muted);font-size:12px">No image uploaded</div>
                    @endif
                </div>
            ...
            <div style="display:flex;flex-wrap:wrap;gap:8px">
                ...
                <div style="background:rgba(212,160,23,.1);border:1px solid rgba(212,160,23,.2);border-radius:8px;padding:6px 12px;display:flex;align-items:center;gap:8px">
                    ...
                    <div>
                        ...
                        <div style="color:var(--cms-text-muted);font-size:10px">{{ $zone->x_percent }}%, {{ $zone->y_percent }}% — r:{{ $zone->radius_percent }}%</div>
                    </div>
                </div>
                @endforeach
            </div>
