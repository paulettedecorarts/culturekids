<div class="sd-manager-page">
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:var(--sp-5);gap:var(--sp-3);flex-wrap:wrap">
        <div>
            <div class="sa-page-title">Spot the Difference</div>
            <div class="sa-breadcrumb">Visual comparison activities with cultural scenes</div>
        </div>
        <a href="{{ route($routePrefix . '.spot-differences.create') }}" class="btn btn-primary" style="padding:12px 28px;border-radius:14px;font-weight:800;font-size:13px;box-shadow:0 8px 24px rgba(196,75,43,0.3);text-decoration:none">
            + Create Activity
        </a>
    </div>
    ...
    <div class="sa-stats-row" style="grid-template-columns:repeat(4,minmax(0,1fr));gap:var(--sp-3);margin-bottom:var(--sp-4)">
        ...
        <div class="sa-stat">
            <div class="sa-stat-val">{{ \App\Models\SpotDifference::where('status','published')->count() }}</div>
            <div class="sa-stat-label">Published</div>
        </div>
        <div class="sa-stat">
            <div class="sa-stat-val">{{ \App\Models\SpotDifference::where('difficulty_level','easy')->count() }}</div>
            <div class="sa-stat-label">Easy</div>
        </div>
        <div class="sa-stat">
            <div class="sa-stat-val">{{ \App\Models\SpotDifference::where('difficulty_level','hard')->count() }}</div>
            <div class="sa-stat-label">Hard</div>
        </div>
    </div>
            ...
            <div class="sa-table-row" style="display:grid;grid-template-columns:2fr 1fr 1fr 1fr 1fr 1fr;gap:var(--sp-3);padding:12px 16px;border-bottom:1px solid var(--cms-border-subtle);align-items:center">
                <div style="display:flex;align-items:center;gap:12px;min-width:0">
                    ...
                    <div style="min-width:0">
                        ...
                        <div style="font-size:11px;color:var(--cms-text-muted)">{{ $activity->scene_name ?: 'No scene name' }} • Ages {{ $activity->age_range }}</div>
                    </div>
                </div>
                ...
                <span style="font-size:12px;color:var(--cms-text-muted)">{{ $activity->zones_count }} zones</span>
                <div style="display:flex;gap:6px">
                    <a href="{{ route($routePrefix . '.spot-differences.show', $activity->id) }}" class="btn btn-sm" style="background:rgba(212,160,23,.18);color:#F2CB5A;border:1px solid rgba(212,160,23,.3);padding:4px 10px;border-radius:var(--r-full);font-size:10px;font-weight:700;text-decoration:none">View</a>
                    <a href="{{ route($routePrefix . '.spot-differences.edit', $activity->id) }}" class="btn btn-sm" style="background:rgba(59,130,246,.18);color:#60A5FA;border:1px solid rgba(59,130,246,.3);padding:4px 10px;border-radius:var(--r-full);font-size:10px;font-weight:700;text-decoration:none">Edit</a>
                </div>
            </div>
        @empty
            <div style="padding:40px;text-align:center;color:var(--cms-text-muted)">
                <div style="font-size:48px;margin-bottom:16px">🔍</div>
                <div style="font-size:16px;font-weight:600;margin-bottom:8px">No activities yet</div>
                <a href="{{ route($routePrefix . '.spot-differences.create') }}" class="btn btn-primary" style="text-decoration:none">Create First Activity</a>
            </div>
        @endforelse
    </div>
