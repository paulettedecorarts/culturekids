<div class="song-show-page">
    ...
    <div class="sa-content-card" style="margin-bottom:var(--sp-4)">
            ...
            <div>
                ...
                <div style="display:flex;flex-direction:column;gap:var(--sp-2)">
                    <div style="display:flex;justify-content:space-between">
                        ...
                        <span style="color:var(--cms-text);font-size:12px;font-weight:600">{{ $song->tribe->name ?? 'N/A' }}</span>
                    </div>
                    <div style="display:flex;justify-content:space-between">
                        <span style="color:var(--cms-text-muted);font-size:12px">Language</span>
                        <span style="color:var(--cms-text);font-size:12px;font-weight:600">{{ ucfirst($song->language ?? 'Not specified') }}</span>
                    </div>
                    <div style="display:flex;justify-content:space-between">
                        <span style="color:var(--cms-text-muted);font-size:12px">Age Range</span>
                        <span style="color:var(--cms-text);font-size:12px;font-weight:600">{{ $song->age_range }}</span>
                    </div>
                    <div style="display:flex;justify-content:space-between">
                        <span style="color:var(--cms-text-muted);font-size:12px">Difficulty</span>
                        <span style="color:var(--cms-text);font-size:12px;font-weight:600">{{ ucfirst($song->difficulty_level ?? 'Not specified') }}</span>
                    </div>
                    <div style="display:flex;justify-content:space-between">
                        <span style="color:var(--cms-text-muted);font-size:12px">Star Points</span>
                        <span style="color:var(--cms-text);font-size:12px;font-weight:600">{{ $song->star_points }}</span>
                    </div>
                    <div style="display:flex;justify-content:space-between">
                        ...
                        <span style="padding:2px 8px;border-radius:12px;font-size:10px;font-weight:700;
                            {{ $song->status === 'published' ? 'background:rgba(74,124,89,.2);color:#4A7C59;border:1px solid rgba(74,124,89,.35)' : 
                               ($song->status === 'draft' ? 'background:rgba(212,160,23,.2);color:#F2CB5A;border:1px solid rgba(212,160,23,.45)' : 'background:var(--cms-surface-raised);color:var(--cms-text-muted);border:1px solid var(--cms-border)') }}">
                            {{ ucfirst($song->status) }}
                        </span>
                    </div>
                </div>
                ...
                <div style="display:flex;flex-direction:column;gap:var(--sp-2)">
                    <div style="display:flex;align-items:center;gap:8px">
                        <span style="width:16px;height:16px;display:flex;align-items:center;justify-content:center">
                            ...
                            @else
                                <span style="color:var(--cms-text-muted);font-size:14px">✗</span>
                            @endif
                        </span>
                        <span style="color:var(--cms-text);font-size:12px">Karaoke Timing</span>
                    </div>
                    <div style="display:flex;align-items:center;gap:8px">
                        <span style="width:16px;height:16px;display:flex;align-items:center;justify-content:center">
                        ...
                        <span style="color:var(--cms-text);font-size:12px">Fill-the-Blanks</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div style="margin-bottom:var(--sp-4)">
            ...
            <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:var(--sp-3)">
                <div style="text-align:center">
                    <div style="color:var(--cms-text);font-size:12px;font-weight:600;margin-bottom:8px">Audio</div>
                    @if($song->audio_path)
                    ...
                    @else
                        <p style="color:var(--cms-text-muted);font-size:11px;margin:0">No audio file</p>
                    @endif
                </div>

                <div style="text-align:center">
                    <div style="color:var(--cms-text);font-size:12px;font-weight:600;margin-bottom:8px">Video</div>
                    @if($song->video_path)
                    ...
                    @else
                        <p style="color:var(--cms-text-muted);font-size:11px;margin:0">No video file</p>
                    @endif
                </div>

                <div style="text-align:center">
                    <div style="color:var(--cms-text);font-size:12px;font-weight:600;margin-bottom:8px">Cover Image</div>
                    @if($song->cover_image_path)
                        <img src="{{ asset('storage/' . $song->cover_image_path) }}" alt="Cover" style="width:100%;max-width:200px;height:120px;object-fit:cover;border-radius:8px;border:1px solid var(--cms-border)">
                    @else
                        <div style="width:100%;max-width:200px;height:120px;background:var(--cms-surface-raised);border-radius:8px;border:1px solid var(--cms-border);display:flex;align-items:center;justify-content:center;margin:0 auto">
                            <span style="color:var(--cms-text-muted);font-size:11px">No cover image</span>
                        </div>
                    @endif
                </div>
            </div>
        ...
        <div style="margin-bottom:var(--sp-4)">
            ...
            <p style="color:var(--cms-text);font-size:13px;line-height:1.5">{{ $song->description }}</p>
        </div>
        @endif
        ...
        <div style="margin-bottom:var(--sp-4)">
            ...
            <div style="background:var(--cms-surface-raised);border:1px solid var(--cms-border);border-radius:8px;padding:var(--sp-3)">
                <pre style="white-space:pre-wrap;color: var(--cms-text);font-size:12px;font-family:var(--font-mono);line-height:1.6;margin:0">{{ $song->lyrics }}</pre>
            </div>
        </div>
        @endif
        ...
        <div style="margin-bottom:var(--sp-4)">
            ...
            <div style="display:flex;flex-direction:column;gap:var(--sp-2)">
                ...
                <div style="border:1px solid var(--cms-border);border-radius:8px;padding:var(--sp-3);background:var(--cms-surface)">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">
                        <span style="color:var(--cms-text);font-size:12px;font-weight:600">{{ ucfirst($segment->segment_type) }} #{{ $segment->order_index + 1 }}</span>
                        <span style="color:var(--cms-text-muted);font-size:10px">{{ $segment->start_time }}s - {{ $segment->end_time }}s</span>
                    </div>
                    <p style="color: var(--cms-text);font-size:12px;margin:0">{{ $segment->segment_text }}</p>
            ...
            <div style="background:var(--cms-surface-raised);border:1px solid var(--cms-border);border-radius:8px;padding:var(--sp-3)">
                <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(120px,1fr));gap:var(--sp-3);text-align:center">
                    <div>
                        ...
                        <div style="color:var(--cms-text-muted);font-size:11px">Total Plays</div>
                    </div>
                    <div>
