<div class="drawings-manager-page">
    @php
        $contentRoute = $this->contentRoutePrefix();
        $isColouring = $this->isColouringList();
        $pageTitle = $isColouring ? 'Colouring Pages' : 'Drawing Activities';
        $pageSubtitle = $isColouring
            ? 'Published colouring templates for the child app'
            : 'Coloring pages, hero drawings, design tools, and creative activities';
        $createLabel = $isColouring ? '+ Create Colouring Page' : '+ Create Drawing';
    @endphp
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:var(--sp-5);gap:var(--sp-3);flex-wrap:wrap">
        <div>
            <div class="sa-page-title">{{ $pageTitle }}</div>
            <div class="sa-breadcrumb">{{ $pageSubtitle }}</div>
        </div>
        <a href="{{ route($routePrefix . '.' . $contentRoute . '.create') }}" class="btn btn-primary" style="padding:12px 28px; border-radius:14px; font-weight:800; font-size:13px; box-shadow: 0 8px 24px rgba(196,75,43,0.3); text-decoration:none">
            {{ $createLabel }}
        </a>
    </div>
    ...
    <div class="sa-stats-row" style="grid-template-columns: repeat(4, minmax(0,1fr)); gap: var(--sp-3); margin-bottom: var(--sp-4)">
        <div class="sa-stat">
            ...
            <div class="sa-stat-delta">{{ $isColouring ? 'Colouring pages' : 'All drawings' }}</div>
        </div>
        <div class="sa-stat">
            <div class="sa-stat-val">{{ \App\Models\Drawing::where('status', 'published')->when($isColouring, fn ($q) => $q->whereIn('drawing_type', \App\Support\ActivityDrawingTypeFilter::COLOURING_TYPES))->count() }}</div>
    ...
    <div class="cms-toolbar-flex" style="display:flex;gap:var(--sp-3);margin-bottom:var(--sp-4);flex-wrap:wrap">
        <input wire:model.live.debounce.300ms="search" placeholder="{{ $isColouring ? 'Search colouring pages...' : 'Search drawings...' }}" style="padding:8px 14px;border-radius:var(--r-full);border:1px solid var(--cms-input-border);background:var(--cms-input-bg);color:var(--cms-text);font-family:var(--font-admin);font-size:12px;outline:none;flex:1;min-width:180px">
    ...
    <div class="sa-table-wrap">
        <div class="sa-table-head" style="display:grid;grid-template-columns:2fr 1fr 1fr 1fr 1fr 1fr;gap:var(--sp-3);padding:12px 16px;background:var(--cms-surface-raised);border-radius:8px;font-size:11px;font-weight:700;color:var(--cms-text-muted);text-transform:uppercase;letter-spacing:.5px">
            <span>{{ $isColouring ? 'Colouring page' : 'Drawing' }}</span>
            <span>Type</span>
        ...
        @forelse($this->drawings as $drawing)
            <div class="sa-table-row" style="display:grid;grid-template-columns:2fr 1fr 1fr 1fr 1fr 1fr;gap:var(--sp-3);padding:12px 16px;border-bottom:1px solid var(--cms-border-subtle);align-items:center">
                ...
                <div style="display:flex;gap:6px">
                    <a href="{{ route($routePrefix . '.' . $contentRoute . '.play', $drawing->id) }}" target="_blank" class="btn btn-sm" style="background:rgba(74,124,89,.18);color:#6FA882;border:1px solid rgba(74,124,89,.3);padding:4px 10px;border-radius:var(--r-full);font-size:10px;font-weight:700;text-decoration:none">Play</a>
                    <a href="{{ route($routePrefix . '.' . $contentRoute . '.show', $drawing->id) }}" class="btn btn-sm" style="background:rgba(212,160,23,.18);color:#F2CB5A;border:1px solid rgba(212,160,23,.3);padding:4px 10px;border-radius:var(--r-full);font-size:10px;font-weight:700;text-decoration:none">View</a>
                    <a href="{{ route($routePrefix . '.' . $contentRoute . '.edit', $drawing->id) }}" class="btn btn-sm" style="background:rgba(59,130,246,.18);color:#60A5FA;border:1px solid rgba(59,130,246,.3);padding:4px 10px;border-radius:var(--r-full);font-size:10px;font-weight:700;text-decoration:none">Edit</a>
                </div>
            </div>
        @empty
            <div style="padding:40px;text-align:center;color:var(--cms-text-muted)">
                <div style="font-size:48px;margin-bottom:16px">{{ $isColouring ? '🖍️' : '🎨' }}</div>
                <div style="font-size:16px;font-weight:600;margin-bottom:8px">No {{ $isColouring ? 'colouring pages' : 'drawing activities' }} found</div>
                <div style="font-size:12px;margin-bottom:20px">Create your first {{ $isColouring ? 'colouring page' : 'drawing activity' }} to get started</div>
                <a href="{{ route($routePrefix . '.' . $contentRoute . '.create') }}" class="btn btn-primary" style="text-decoration:none">Create {{ $isColouring ? 'Colouring Page' : 'Drawing Activity' }}</a>
            </div>
        @endforelse
    </div>
