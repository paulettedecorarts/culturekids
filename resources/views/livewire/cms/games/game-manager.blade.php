<div class="game-manager-page">
    ...
    <div style="display:flex;gap:var(--sp-3);margin-bottom:var(--sp-4);flex-wrap:wrap">
        <input wire:model.live.debounce.300ms="search" placeholder="Search games..." style="padding:8px 14px;border-radius:var(--r-full);border:1px solid var(--cms-input-border);background:var(--cms-input-bg);color:var(--cms-text);font-family:var(--font-admin);font-size:12px;outline:none;flex:1;min-width:180px">
            ...
            <div class="sa-table-row" style="display:grid;grid-template-columns:2fr 1fr 1fr 1fr 1fr 1fr;gap:var(--sp-3);padding:12px 16px;border-bottom:1px solid var(--cms-border-subtle);align-items:center">
                <div style="display:flex;align-items:center;gap:12px;min-width:0">
                    ...
                    <div style="min-width:0">
                        <div style="font-weight:700;color:var(--cms-text);font-size:13px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">{{ $game->title }}</div>
                        <div style="font-size:11px;color:var(--cms-text-muted)">Ages {{ $game->age_range }} • {{ $game->time_limit_seconds ? $game->time_limit_seconds.'s limit' : 'No time limit' }}</div>
                    </div>
                </div>
                ...
                <span style="font-size:12px;color:var(--cms-text-muted)">{{ $game->tribe->name }}</span>
                ...
                <span style="font-size:12px;color:var(--cms-text-muted)">{{ $game->questions_count }} questions</span>
