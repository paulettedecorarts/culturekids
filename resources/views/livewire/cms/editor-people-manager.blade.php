    ...
    <div class="cms-asset-table">
        ...
        @forelse($editors as $editor)
            <div class="cms-table-row" style="grid-template-columns:1fr 1.2fr auto; cursor:default">
                <div>
                    <div class="cms-asset-name">{{ $editor->name }}</div>
                    <div class="cms-asset-sub">Joined {{ $editor->created_at->diffForHumans() }}</div>
                </div>
                <div style="font-size:14px; font-weight:600; color:var(--ink-light); word-break:break-word">{{ $editor->email }}</div>
                <div style="display:flex; flex-wrap:wrap; gap:6px; justify-content:flex-end">
                    <span class="status-pill status-published" style="font-size:9px">CMS Editor</span>
                </div>
            </div>
        @empty
            <div style="padding:32px; text-align:center; color:var(--stone); font-weight:600">No CMS editors yet.</div>
        @endforelse
    </div>
    ...
    @if ($showInviteModal)
        ...
        >
            <div
                ...
                role="dialog"
                aria-modal="true"
                aria-labelledby="editor-invite-title"
            >
                <div style="padding:var(--sp-6); border-bottom:1px solid var(--cream-mid); display:flex; align-items:flex-start; justify-content:space-between; gap:var(--sp-4); flex-shrink:0">
                    <div>
                        <h2 id="editor-invite-title" style="font-family:var(--font-display); font-size:22px; font-weight:800; color:var(--ink); margin-bottom:4px">Invite CMS Editor</h2>
                        <div style="font-size:12px; color:var(--stone); font-weight:600; line-height:1.45">
                            Add a new <strong>CMS editor</strong> account. They will receive an email with a link to set their password, then they can sign in to create and manage content.
                        </div>
                    </div>
                    <button
                        type="button"
