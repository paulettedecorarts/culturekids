<div class="puzzles-index-page">
    ...
    <div style="display:flex;gap:var(--sp-3);margin-bottom:var(--sp-4);flex-wrap:wrap">
        <input wire:model.live.debounce.300ms="search" placeholder="Search puzzles…" style="padding:8px 14px;border-radius:var(--r-full);border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.06);color:#fff;font-family:var(--font-admin);font-size:12px;outline:none;flex:1;min-width:180px">
            ...
            <div class="sa-table-row puzzle-table-grid">
                <div style="display:flex;align-items:center;gap:12px;min-width:0">
                    ...
                    <div style="min-width:0">
                        <div style="font-weight:700;color:#fff;font-size:13px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">{{ $puzzle->title }}</div>
                        <div style="font-size:11px;color:rgba(255,255,255,.3);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">{{ $puzzle->description ?: 'No description' }}</div>
                    </div>
                </div>
                <span style="font-size:12px;color:rgba(255,255,255,.6)">{{ $puzzle->tribe->name }}</span>
                <span class="status-pill {{ $puzzle->is_published ? 'status-published' : 'status-draft' }}">{{ $puzzle->is_published ? 'Published' : 'Draft' }}</span>
                <span style="font-size:12px;color:rgba(255,255,255,.6)">{{ $puzzle->age_range ?: '—' }}</span>
                <div style="display:flex;gap:6px;flex-wrap:wrap">
                    ...
                    <a href="{{ route($routePrefix . '.puzzles.edit', $puzzle->id) }}" class="btn btn-sm" style="background:rgba(255,255,255,.06);color:rgba(255,255,255,.85);border:1px solid rgba(255,255,255,.12);padding:4px 10px;border-radius:var(--r-full);font-size:10px;font-weight:700;text-decoration:none">Edit</a>
                </div>
            </div>
        @empty
            <div style="padding:22px;color:rgba(255,255,255,.5)">No puzzles yet. Create one to get started.</div>
        @endforelse
    </div>
    ...
    <style>
        ...
        .puzzles-index-page select { background:#1a2744; color:#fff; color-scheme: dark; }
        .puzzles-index-page select option, .puzzles-index-page select optgroup { background:#1a2744; color:#fff; }
