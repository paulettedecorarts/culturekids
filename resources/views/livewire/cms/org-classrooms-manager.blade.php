<div>
        ...
        <div class="cms-asset-table">
            ...
            @forelse($classrooms as $room)
                <div class="cms-table-row" style="grid-template-columns:1fr 1fr minmax(80px, auto) auto; cursor:default; align-items:start">
                    ...
                    <div style="font-size:14px; font-weight:600; color:var(--ink-light)">
                        {{ $room->teacher?->name ?? '—' }}
                    </div>
                    <div style="text-align:center; font-size:14px; font-weight:700; color:var(--ink)">
                        {{ $room->children->count() }}
                    </div>
            ...
            @empty
                <div style="padding:32px; text-align:center; color:var(--stone); font-weight:600">No classrooms yet. Create one to assign a teacher and add children.</div>
            @endforelse
        </div>
    @endif
        ...
        >
            ...
            >
                <div style="padding:var(--sp-6); border-bottom:1px solid var(--cream-mid); display:flex; align-items:flex-start; justify-content:space-between; gap:var(--sp-4); flex-shrink:0">
                    <div>
                        <h2 id="class-modal-title" style="font-family:var(--font-display); font-size:22px; font-weight:800; color:var(--ink); margin-bottom:4px">
                            {{ $editingClassroomId ? __('Edit classroom') : __('New classroom') }}
                        </h2>
                        <div style="font-size:12px; color:var(--stone); font-weight:600">
                ...
                <form wire:submit.prevent="saveClassroom" style="padding:var(--sp-6); overflow-y:auto; display:flex; flex-direction:column; gap:var(--sp-4)">
                    <div>
                        <label style="display:block; font-size:11px; font-weight:700; text-transform:uppercase; color:var(--stone); margin-bottom:6px">{{ __('Name') }}</label>
                        <input type="text" wire:model="formName" autocomplete="off" style="width:100%; padding:10px 12px; border-radius:10px; border:1px solid var(--cream-mid); font-family:var(--font-admin); font-size:15px">
                        @error('formName') <div style="color:var(--clay-red); font-size:12px; margin-top:6px; font-weight:600">{{ $message }}</div> @enderror
                    </div>
                    <div>
                        <label style="display:block; font-size:11px; font-weight:700; text-transform:uppercase; color:var(--stone); margin-bottom:6px">{{ __('Description') }} <span style="font-weight:500">({{ __('optional') }})</span></label>
                        <textarea wire:model="formDescription" rows="3" style="width:100%; padding:10px 12px; border-radius:10px; border:1px solid var(--cream-mid); font-family:var(--font-admin); font-size:15px; resize:vertical"></textarea>
                        @error('formDescription') <div style="color:var(--clay-red); font-size:12px; margin-top:6px; font-weight:600">{{ $message }}</div> @enderror
                    </div>
                    <div>
                        <label style="display:block; font-size:11px; font-weight:700; text-transform:uppercase; color:var(--stone); margin-bottom:6px">{{ __('Class teacher') }}</label>
                        <select
                            wire:model.live="formTeacherId"
                            style="width:100%; padding:10px 12px; border-radius:10px; border:1px solid var(--cream-mid); font-family:var(--font-admin); font-size:15px; background:#fff"
                        >
                        ...
                        @if($teachers->isEmpty())
                            <div style="margin-top:8px; font-size:12px; color:var(--stone); font-weight:600">{{ __('No teachers in your organisation yet.') }}</div>
                        @endif
                    </div>
    ...
    @if ($organization && $showStudentsModal && $studentsClassroomId)
        ...
        >
            <div
                onclick="event.stopPropagation()"
                style="background:#fff; width:100%; max-width:520px; border-radius:var(--r-xl); border:1px solid var(--cream-mid); box-shadow:0 24px 64px rgba(26,18,8,.18); max-height:90vh; display:flex; flex-direction:column; overflow:hidden;"
                role="dialog"
                aria-modal="true"
            ...
            >
                <div style="padding:var(--sp-6); border-bottom:1px solid var(--cream-mid); display:flex; align-items:flex-start; justify-content:space-between; gap:var(--sp-4); flex-shrink:0">
                    <div>
                        <h2 id="students-modal-title" style="font-family:var(--font-display); font-size:22px; font-weight:800; color:var(--ink); margin-bottom:4px">{{ __('Children in class') }}</h2>
                        <div style="font-size:12px; color:var(--stone); font-weight:600">
                            {{ $room?->name ?? '' }}
                        </div>
                    </div>
                ...
                <div style="padding:var(--sp-6); overflow-y:auto; display:flex; flex-direction:column; gap:var(--sp-3)">
                    @if ($orgChildren->isEmpty())
                        <div style="font-size:13px; color:var(--stone); font-weight:600">
                            {{ __('No child accounts in your organisation yet. Invite them from') }}
                            <a href="{{ route('cms.admin.people') }}" wire:navigate style="color:var(--ink); text-decoration:underline">{{ __('Teachers & children') }}</a>.
                        </div>
                    @else
                        <p style="font-size:13px; color:var(--stone); font-weight:600; margin:0">{{ __('Select which children are in this class.') }}</p>
                        <div style="display:flex; flex-direction:column; gap:10px; max-height:min(50vh, 360px); overflow-y:auto; padding-right:4px">
                            @foreach($orgChildren as $stu)
                                <label style="display:flex; align-items:center; gap:12px; padding:10px 12px; border-radius:10px; border:1px solid var(--cream-mid); cursor:pointer; font-size:14px; font-weight:600; color:var(--ink)">
                                    <input type="checkbox" wire:model="selectedStudentIds" value="{{ $stu->id }}" style="width:18px; height:18px">
                                    <span>{{ $stu->name }} <span style="font-weight:500; color:var(--stone)">({{ $stu->email }})</span></span>
                                </label>
                            @endforeach
                        </div>
                ...
                <div style="padding:var(--sp-6); padding-top:0; display:flex; justify-content:flex-end; gap:12px; border-top:1px solid var(--cream-mid); margin-top:auto; flex-shrink:0">
