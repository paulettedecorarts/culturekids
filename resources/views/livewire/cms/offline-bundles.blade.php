    ...
    <div class="sa-table-wrap">
        ...
        @forelse ($comics as $comic)
            <div class="sa-table-row" style="display:grid;grid-template-columns:2fr 1fr 1fr 120px;gap:var(--sp-3);align-items:center">
                <div>
                    <div style="font-weight:700;color:var(--cms-text);font-size:14px">{{ $comic->title }}</div>
                    <div style="font-size:11px;color:var(--cms-text-muted)">{{ $comic->tribe?->name ?? '—' }}</div>
                </div>
                ...
                <div style="font-size:12px;color: var(--cms-text-muted)">{{ $comic->updated_at?->diffForHumans() }}</div>
                <div>
        ...
        @empty
            <div style="padding:var(--sp-8);text-align:center;color:var(--cms-text-muted);font-size:14px">No published story packs yet.</div>
        @endforelse
    </div>
</div>
