<div class="maze-manager-page">
    ...
    <div style="display:flex;gap:var(--sp-3);margin-bottom:var(--sp-4);flex-wrap:wrap">
        <input wire:model.live.debounce.300ms="search" placeholder="Search mazes..." style="padding:8px 14px;border-radius:var(--r-full);border:1px solid rgba(255,255,255,.12);background:rgba(255,255,255,.06);color:#fff;font-family:var(--font-admin);font-size:12px;outline:none;flex:1;min-width:180px">
    ...
    <div class="sa-table-wrap">
        <div class="sa-table-head" style="display:grid;grid-template-columns:2fr 1fr 1fr 1fr 1fr 1fr 1fr;gap:var(--sp-3);padding:12px 16px;background:rgba(255,255,255,.04);border-radius:8px;font-size:11px;font-weight:700;color:rgba(255,255,255,.6);text-transform:uppercase;letter-spacing:.5px">
            <span>Maze</span>
            <span>Type</span>
            <span>Tribe</span>
        ...
        @forelse($this->mazes as $maze)
            <div class="sa-table-row" style="display:grid;grid-template-columns:2fr 1fr 1fr 1fr 1fr 1fr 1fr;gap:var(--sp-3);padding:12px 16px;border-bottom:1px solid rgba(255,255,255,.06);align-items:center">
                <div style="display:flex;align-items:center;gap:12px;min-width:0">
                    <div style="font-size:20px;width:32px;text-align:center">{{ $maze->maze_type_icon }}</div>
                    <div style="min-width:0">
                        <div style="font-weight:700;color:#fff;font-size:13px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">{{ $maze->title }}</div>
                        <div style="font-size:11px;color:rgba(255,255,255,.4)">{{ $maze->grid_rows }}×{{ $maze->grid_cols }} grid • Ages {{ $maze->age_range }}</div>
                    </div>
                </div>
                ...
                <span style="font-size:12px;color:rgba(255,255,255,.6)">{{ $maze->tribe->name }}</span>
                <span style="font-size:12px;color:rgba(255,255,255,.6)">{{ $maze->difficulty_label }}</span>
                ...
                <span style="font-size:12px;color:rgba(255,255,255,.6)">{{ $maze->attempts_count }}</span>
